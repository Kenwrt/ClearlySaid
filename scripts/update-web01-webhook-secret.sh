...
backup_name="clearlysaid-web-rollback-webhook-20260810"
...
secret_fragment="/home/ken/clearlysaid/secrets/webhook-secret.env.new"
env_backup="/home/ken/clearlysaid/secrets/web.env.pre-webhook-fix-20260810"
merged_env="${web_env}.webhook-merge"

test -f "${web_env}"
test -f "${database_env}"
test -f "${secret_fragment}"
...
awk '!/^Stripe__WebhookSecret=/' "${web_env}" > "${merged_env}"
cat "${secret_fragment}" >> "${merged_env}"
chmod 600 "${merged_env}"

if [[ "$(grep -c '^Stripe__WebhookSecret=' "${merged_env}")" -ne 1 ]]; then
  echo "Expected exactly one Stripe webhook secret." >&2
  exit 1
fi
...
rollback() {
  echo "Webhook-secret update failed; restoring the prior container and environment." >&2
...
rm -f "${secret_fragment}"
trap - ERR
echo "ClearlySaid webhook signing secret updated; Web01 is healthy."
